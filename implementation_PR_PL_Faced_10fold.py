...
from model_PR_PL_Faced import Domain_adaption_model,discriminator
...
class StepwiseLR_GRL: 
    ...
    def __init__(self, optimizer: Optimizer, init_lr: Optional[float] = 0.01,
                 gamma: Optional[float] = 0.001, decay_rate: Optional[float] = 0.75,max_iter: Optional[float] = 14):
...
def modi_load_data(test_id, session):
    Faced_Labels_3 = np.array([0] * 12 + [1] * 4 + [2] * 12)
    Faced_Labels_9 = np.array([0] * 3 + [1] * 3 + [2] * 3 + [3] * 3 + [4] * 4 + [5] * 3 + [6] * 3 + [7] * 3 + [8] * 3)
    DE_Dir = './data/Faced/DE_fold'
    test_id = f'fold_{test_id+1}.pkl'
    sub_de_dir = os.path.join(DE_Dir, test_id)
    with open(sub_de_dir, 'rb') as file:
        de_matrix = pickle.load(file)
        # (28, 32, 360, 5) (vid, channel, slice, freq)
        de_matrix = np.transpose(de_matrix, (0,2,1,3))
        # (28, 360, 32, 5) (vid, slice, channel, freq)
        shape = de_matrix.shape
        de_matrix = de_matrix.reshape(shape[0]*shape[1], shape[2]*shape[3])
        matrices = de_matrix
        n_class = 9
        if(n_class == 9):
            labels = np.zeros((28, 360, 9))
            for i_vid in range(28):
                for i_slice in range(360):
                    labels[i_vid][i_slice][Faced_Labels_9[i_vid]] =  1
            labels = labels.reshape(matrices.shape[0], 9)
        
        if(n_class == 3):
            labels = np.zeros((28, 360, 3))
            for i_vid in range(28):
                for i_slice in range(360):
                    labels[i_vid][i_slice][Faced_Labels_3[i_vid]] =  1
            labels = labels.reshape(matrices.shape[0], 3)
...
#    path='F:\\zhourushuang\\transfer_learning\\feature_for_net_session1_LDS_de'
    ...
    for sub in range(10):
        sub_id = sub + 1
        feature, one_hot_label_mat = modi_load_data(sub, session)
        feature_list.append(min_max_scaler.fit_transform(feature).astype('float32')) # Variable 'feature' is a [3394, 310] DE feature matrix from SEED dataset.
        label_list.append(one_hot_label_mat.astype('float32'))
    # for feature_i in label_list:
...
def train_model(loader_train, loader_test,model,dann_loss, optimizer,scheduler,hidden_4,epoch,batch_size,parameter,threshold_update=True): ## model training 
    ...
    for i in range(T):  
        ...
        # cls_loss: Classification loss for source data (labeled)
        # cluster_loss: Cluster loss for target data (unlabeled)
        # transfer_loss: Domain adversarial loss
        # P_loss: maybe prototype loss?
...
def main(update_threshold,parameter,session):
    ...
    setup_seed(20)
    max_iter=14
    best_acc_mat=np.zeros(10)
    transfer_loss_curve=np.zeros((10,max_iter))
    cls_loss_curve=np.zeros((10,max_iter))
    source_acc_curve=np.zeros((10,max_iter))
    target_acc_curve=np.zeros((10,max_iter))
    source_nmi_curve=np.zeros((10,max_iter))
    target_nmi_curve=np.zeros((10,max_iter))
    for i in range(10):
        print(f'fold {i}/10')
...
# ## hyperparameter setting for the Faced dataset    
parameter={'hidden_1':64,'hidden_2':64,'num_of_class':9,'cluster_weight':2,'low_rank':32,'upper_threshold':0.9,'lower_threshold':0.5,'boost_type':'linear'}
...
def save_dict_to_file(dictionary, directory, filename):
...
# 指定目录和文件名
directory = './result/Faced/9_class_10_fold'  # 替换为你的目录路径
filename = 'result.pkl'  # 你想要保存的文件名

# 保存字典到文件
save_dict_to_file(result_list, directory, filename)
