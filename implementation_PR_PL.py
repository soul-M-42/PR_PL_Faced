...
def modi_load_data(test_id, session):
    vid_label = [1,0,-1,-1,0,1,-1,0,1,1,0,-1,0,1,-1]
    test_id = test_id + 1
    test_id_str = str(test_id)
    file_prefix = f"{test_id_str}_"

    # 查找以指定test_id开头的所有文件
    files = [f for f in os.listdir('./data/SEED/LDS_DE/ExtractedFeatures') if f.startswith(file_prefix)]

    if len(files) == 0:
        print(f"未找到以 {file_prefix} 开头的文件")
        return None
    file = files[session-1]
    matrices = np.zeros((0, 310))
    labels = np.zeros((0, 3))
    file_path = os.path.join('./data/SEED/LDS_DE/ExtractedFeatures', file)
    ...
    for vid in range(1,16):
        matrix_key = f"de_LDS{vid}"
        if matrix_key in data:
            data_i = np.transpose(data[matrix_key], (1, 0, 2))
            data_i =  data_i.reshape(data_i.shape[0], -1)
            matrices = np.concatenate((matrices, data_i))
            label_onehot = np.zeros((1, 3))
            label_onehot[0, vid_label[vid-1] + 1] = 1
            label_onehot = np.tile(label_onehot, (data_i.shape[0], 1))
            # print(label_onehot)
            labels = np.concatenate((labels, label_onehot))
        else:
            print(f"在文件 {file} 中未找到矩阵 {matrix_key}")
...
# 指定目录和文件名
directory = './result/SEED/3_class'  # 替换为你的目录路径
