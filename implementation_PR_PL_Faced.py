...
class StepwiseLR_GRL: 
    ...
    def __init__(self, optimizer: Optimizer, init_lr: Optional[float] = 0.01,
                 gamma: Optional[float] = 0.001, decay_rate: Optional[float] = 0.75,max_iter: Optional[float] = 500):
...
def modi_load_data(test_id, session):
    ...
    with open(sub_de_dir, 'rb') as file:
        ...
        # (28, 32, 30, 5) (vid, channel, slice, freq)
        if not lds:
            de_matrix = np.transpose(de_matrix, (0,2,1,3))
        # (28, 30, 32, 5) (vid, slice, channel, freq)
        
        # de_matrix = de_matrix.reshape(28, 30, 32*5)
        # for i_vid in range(28):
        #     de_matrix[i_vid] = LDS(de_matrix[i_vid])
        #     # de_matrix[i_vid] = de_matrix[i_vid]
        #     continue
        de_matrix = de_matrix.reshape(28*30, 32*5)
        matrices = de_matrix
        n_class = 9
        if(n_class == 9):
            labels = np.zeros((28, 30, 9))
            for i_vid in range(28):
                for i_slice in range(30):
                    labels[i_vid][i_slice][Faced_Labels_9[i_vid]] =  1
            labels = labels.reshape(matrices.shape[0], 9)
        
        if(n_class == 3):
            labels = np.zeros((28, 30, 3))
            for i_vid in range(28):
                for i_slice in range(30):
...
def main(update_threshold,parameter,session):
    ...
    setup_seed(20)
    max_iter=500
    best_acc_mat=np.zeros(123)
    transfer_loss_curve=np.zeros((123,max_iter))
    cls_loss_curve=np.zeros((123,max_iter))
    source_acc_curve=np.zeros((123,max_iter))
    target_acc_curve=np.zeros((123,max_iter))
    source_nmi_curve=np.zeros((123,max_iter))
    target_nmi_curve=np.zeros((123,max_iter))
    for i in range(123):
        print(f'Sub {i}/123')
...
# 指定目录和文件名
directory = './result/Faced/9_class'  # 替换为你的目录路径
